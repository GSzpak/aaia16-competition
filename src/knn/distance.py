...
def multidimensional_dynamic_time_warping(md_time_series1, md_time_series2, distance_fun=abs):
    ...
    len1 = md_time_series1.shape[1]
    len2 = md_time_series2.shape[1]
    print len1, len2, num_of_dimensions
    distances = np.zeros((len1, len2))
    for i in xrange(len1):
        for j in xrange(len2):
            distances[i, j] = sum([distance_fun(md_time_series1[k, i] - md_time_series2[k, j])
